...
// @namespace    https://github.com/ilovelogo/suno-zhongwentong
// @version      1.6.0
...
// @homepageURL  https://github.com/ilovelogo/suno-zhongwentong
// @supportURL   https://github.com/ilovelogo/suno-zhongwentong/issues
// @downloadURL  https://raw.githubusercontent.com/ilovelogo/suno-zhongwentong/main/dist/suno-copilot.user.js
// @updateURL    https://raw.githubusercontent.com/ilovelogo/suno-zhongwentong/main/dist/suno-copilot.user.js
...
const UI_SENTENCES = {
  ...
  // ==========================================
  // 15. Labs 实验工坊与 Listen and Rank (听歌评分赚积分)
  // ==========================================
  'Listen for Credits': '听歌评测赚积分',
  ...
  'Two clips, A and B. Listen to both, then pick the one that better answers: Which clip better matches the prompt? — or \'Neither\' if they tie or both fail.': '有两个片段 A 和 B。完整听完两首后，选出更符合要求的一首；如果打平或都不符合，请选“都不符合”。',
  'Two clips, A and B. Listen to both, then pick the one that better answers the prompt. If you\'re not sure, pick either one. Every response helps Suno improve!': '片段 A 与 B。请试听两者，并选出更能体现提示词的片段。如果不确定，任选其一即可。每一次回答都能帮助 Suno 进化！',
  'Clip A': '片段 A',
  'Clip B': '片段 B',
  ...
  'Listen and Rank': '试听打分评测',
  'Earn credits': '赚取积分',
  'Earn 5 credits': '赚取 5 积分',
  'Optional: explain your selection(s)': '选填：写下你选择该片段的理由',
  'explain your selection(s)': '写下你选择该片段的理由',
  'Submit': '提交',
  'play/pause': '播放/暂停',
  'navigate': '切换选项',
  'Genre Wheel': '曲风大转盘',
  ...
  'Apple Account': 'Apple 账号',
  'Discord Account': 'Discord 账号',
  'Microsoft Account': 'Microsoft 账号',
  'Connect': '绑定',
  'Disconnect': '解绑',
  'Password': '登录密码',
  ...
  'Are you sure you want to delete your account?': '你确定要彻底注销此账号吗？此操作无法撤销。',

  // ==========================================
  // 18. Subscription & Billing 会员订阅与账单
  // ==========================================
  ...
  'Renew Subscription': '续订会员',
  'Add Payment Method': '添加支付方式',
  'Invoices': '发票与收据',
  'Download Invoice': '下载发票',

  // ==========================================
  // 19. Audio Preferences & Privacy 播放偏好与隐私
  // ==========================================
  ...
  'Save Settings': '保存设置',
  'Changes saved successfully': '设置已成功保存'
};
...
class SunoCopilotEngine {
  ...
  scanNavigationAndMenus() {
    const candidates = document.querySelectorAll(
      ...
      'h1, h2, h3, h4, h5, h6, label, span, p, [class*="label"], [class*="title"], [class*="setting"], ' +
      '[class*="param"], [class*="control"], [class*="option"], [class*="item"], ' +
      '[class*="desc"], [class*="instruction"], [class*="subtitle"], [class*="badge"], [class*="header"]'
    );

    for (const el of candidates) {
      if (this.isEditableOrIgnored(el)) continue;

      const existingNav = el.querySelector && el.querySelector('.suno-copilot-nav-zh');
      if (el.dataset && el.dataset.scNavProcessed && !el.dataset.scDynamicNav) continue;
      ...
      if (existingNav && !el.dataset.scDynamicNav) {
        continue;
      }
      ...
      // 动态试听倒计时匹配：Listen for 2 more seconds / Listen for 1 more second
      if (!zh) {
        const countdownMatch = rawText.match(/^Listen for (\d+) more seconds?$/i);
        if (countdownMatch) {
          zh = `还需试听 ${countdownMatch[1]} 秒`;
          isDynamic = true;
        }
      }

      // 动态赚取积分匹配：Earn 5 credits / +5 Credits
      if (!zh) {
        const creditsMatch = rawText.match(/^(?:Earn|\+)?\s*(\d+)\s*Credits?$/i);
        if (creditsMatch) {
          zh = `赚取 ${creditsMatch[1]} 积分`;
          isDynamic = true;
        }
      }
      ...
      if (zh) {
        if (existingNav) {
          if (existingNav.textContent !== zh) {
            existingNav.textContent = zh;
          }
          continue;
        }

        if (el.dataset) {
          el.dataset.scNavProcessed = 'true';
          if (isDynamic) el.dataset.scDynamicNav = 'true';
        }
        el.title = `${rawText} · ${zh}`;
        ...
        } else if (el.tagName === 'P' || zh.length > 30 || rawText.length > 40) {
          navSpan.classList.add('block');
        }
  ...
  scanPlaceholders() {
    ...
    for (const el of inputs) {
      ...
      if (trimmedPh.toLowerCase().includes('explain your selection') && !trimmedPh.includes('选填：写下你选择该片段的理由')) {
        el.placeholder = 'Optional: explain your selection(s) · 选填：写下你选择该片段的理由';
        if (el.setAttribute) el.setAttribute('placeholder', el.placeholder);
        if (el.dataset) el.dataset.scPhProcessed = 'true';
        continue;
      }
      if (trimmedPh.toLowerCase().includes('tell the world about yourself') && !trimmedPh.includes('用一句话介绍你自己')) {
        el.placeholder = 'Tell the world about yourself · 用一句话介绍你自己';
        if (el.setAttribute) el.setAttribute('placeholder', el.placeholder);
        if (el.dataset) el.dataset.scPhProcessed = 'true';
        continue;
      }
      if (trimmedPh.toLowerCase().includes('your handle and profile url') && !trimmedPh.includes('你的个性标识与主页地址')) {
        el.placeholder = 'Your handle and profile URL · 你的个性标识与主页地址';
        if (el.setAttribute) el.setAttribute('placeholder', el.placeholder);
        if (el.dataset) el.dataset.scPhProcessed = 'true';
        continue;
      }
