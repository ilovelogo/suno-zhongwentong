...
(async () => {
  ...
  // ==============================================================
  // 断言 8：【歌曲列表灰色描述 100% 本地专业整句翻译（无英文残留）】
  // ==============================================================
  {
    const songCard = new MockElement('div', { className: 'song-card' });
    const songTitle = new MockElement('div', { textContent: 'My Cozy Tune' });
    const songDesc = new MockElement('p', { textContent: 'soft intimate vocal, acoustic guitar, slow' });
    ...
    assert.strictEqual(zhDescEl.textContent, '温柔贴耳人声 · 原声木吉他 · 慢速', 'FAIL: 歌曲描述翻译不符合规范整句');
    assert(!/[a-zA-Z]/.test(zhDescEl.textContent), 'FAIL: 中文行绝对不可残留英文字母！');
    assert.strictEqual(songDesc.textContent, 'soft intimate vocal, acoustic guitar, slow', 'FAIL: 原始英文字符被篡改！');

    console.log('✅ 断言 8 通过: soft intimate vocal, acoustic guitar, slow 译为纯中文人话，零英文字符残留。');
    testsPassed++;
  }

  // ==============================================================
  // 断言 9：【未收录的未知英文描述绝对不会把英文碎片拼进中文行】
  // ==============================================================
  {
    ...
    assert.strictEqual(zhDescEl2, null, 'FAIL: 无法确定翻译的句子严禁生成残留英文碎片的中文行，应直接保持空！');

    console.log('✅ 断言 9 通过: 无法完整翻译的句子不拼凑夹生饭，绝不将英文碎片泄露入中文行。');
    testsPassed++;
  }
  ...
  // ==============================================================
  // 断言 12：【源代码与产物中 100% 杜绝 translate.googleapis.com 网络通道】
  // ==============================================================
  {
    ...
    assert(!injectorSrc.includes('translate.googleapis.com'), 'FAIL: injector.js 仍残留 translate.googleapis.com');
    assert(!buildSrc.includes('translate.googleapis.com'), 'FAIL: build.js 仍残留 translate.googleapis.com');
    assert(!manifestSrc.includes('translate.googleapis.com'), 'FAIL: manifest.json 仍残留 translate.googleapis.com');
    assert(!manifestSrc.includes('background.js'), 'FAIL: manifest.json 仍包含 background service worker');
    assert(!fs.existsSync(path.join(__dirname, '../dist/chrome-extension/background.js')), 'FAIL: background.js 文件仍存在于扩展包中！');

    console.log('✅ 断言 12 通过: 谷歌翻译通道、网络请求、host_permissions 与 background.js 已 100% 彻底拆除。');
    testsPassed++;
  }
