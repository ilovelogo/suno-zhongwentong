...
(async () => {
  ...
  // ==============================================================
  // 断言 16：【全界面所有面板、弹窗、折叠栏与上下文菜单高级参数汉化全覆盖】
  // 包括 Sounds 声音模式、Upload Audio 模态框、Extend 续写面板、Audio Editor 编辑面板、
  // Stems 分轨弹窗、歌曲上下文菜单、Library 音乐库管理、Settings 账户中心
  // ==============================================================
  {
    const multiPanelsRoot = new MockElement('div', { className: 'multi-panels-container' });

    // 1. Sounds 声音面板高级参数
    const soundsModal = new MockElement('div', { className: 'sounds-settings' });
    const loopBtn = new MockElement('button', { textContent: 'Loop' });
    const oneShotBtn = new MockElement('button', { textContent: 'One-shot' });
    const pitchLabel = new MockElement('label', { textContent: 'Pitch:' });
    const keyLabel = new MockElement('label', { textContent: 'Key:' });
    const bpmLabel = new MockElement('label', { textContent: 'BPM:' });
    soundsModal.appendChild(loopBtn);
    soundsModal.appendChild(oneShotBtn);
    soundsModal.appendChild(pitchLabel);
    soundsModal.appendChild(keyLabel);
    soundsModal.appendChild(bpmLabel);

    // 2. Upload Audio 模态框
    const uploadModal = new MockElement('div', { className: 'upload-dialog' });
    const uploadHeader = new MockElement('h2', { textContent: 'Upload Audio' });
    const recordBtn = new MockElement('button', { textContent: 'Record Audio' });
    const chooseFileBtn = new MockElement('button', { textContent: 'Choose File' });
    const startTimeLabel = new MockElement('label', { textContent: 'Start Time:' });
    const endTimeLabel = new MockElement('label', { textContent: 'End Time:' });
    uploadModal.appendChild(uploadHeader);
    uploadModal.appendChild(recordBtn);
    uploadModal.appendChild(chooseFileBtn);
    uploadModal.appendChild(startTimeLabel);
    uploadModal.appendChild(endTimeLabel);

    // 3. Extend 续写面板
    const extendModal = new MockElement('div', { className: 'extend-dialog' });
    const extendAtLabel = new MockElement('label', { textContent: 'Extend At:' });
    const fullSongBtn = new MockElement('button', { textContent: 'Get Whole Song' });
    const part1Btn = new MockElement('button', { textContent: 'Part 1' });
    extendModal.appendChild(extendAtLabel);
    extendModal.appendChild(fullSongBtn);
    extendModal.appendChild(part1Btn);

    // 4. Audio Editor 编辑面板
    const editorModal = new MockElement('div', { className: 'editor-dialog' });
    const cropBtn = new MockElement('button', { textContent: 'Crop Audio' });
    const replaceBtn = new MockElement('button', { textContent: 'Replace Section' });
    const speedLabel = new MockElement('label', { textContent: 'Playback Speed:' });
    const fadeInBtn = new MockElement('button', { textContent: 'Fade In' });
    const fadeOutBtn = new MockElement('button', { textContent: 'Fade Out' });
    editorModal.appendChild(cropBtn);
    editorModal.appendChild(replaceBtn);
    editorModal.appendChild(speedLabel);
    editorModal.appendChild(fadeInBtn);
    editorModal.appendChild(fadeOutBtn);

    // 5. Stems 分轨提取面板
    const stemsModal = new MockElement('div', { className: 'stems-dialog' });
    const getStemsBtn = new MockElement('button', { textContent: 'Get Stems' });
    const vocalStemLabel = new MockElement('span', { textContent: 'Vocal Stem' });
    const instStemLabel = new MockElement('span', { textContent: 'Instrumental Stem' });
    stemsModal.appendChild(getStemsBtn);
    stemsModal.appendChild(vocalStemLabel);
    stemsModal.appendChild(instStemLabel);

    // 6. 歌曲上下文操作菜单
    const contextMenu = new MockElement('div', { className: 'context-menu' });
    const reusePromptBtn = new MockElement('button', { textContent: 'Reuse Prompt' });
    const coverSongBtn = new MockElement('button', { textContent: 'Cover Song' });
    const downloadAudioBtn = new MockElement('button', { textContent: 'Download Audio' });
    const shareBtn = new MockElement('button', { textContent: 'Share' });
    const moveToTrashBtn = new MockElement('button', { textContent: 'Move to Trash' });
    contextMenu.appendChild(reusePromptBtn);
    contextMenu.appendChild(coverSongBtn);
    contextMenu.appendChild(downloadAudioBtn);
    contextMenu.appendChild(shareBtn);
    contextMenu.appendChild(moveToTrashBtn);

    // 7. Library 音乐库与批量管理面板
    const libraryPanel = new MockElement('div', { className: 'library-panel' });
    const playlistsBtn = new MockElement('button', { textContent: 'Playlists' });
    const likedBtn = new MockElement('button', { textContent: 'Liked' });
    const sortByLabel = new MockElement('label', { textContent: 'Sort by:' });
    const restoreAllBtn = new MockElement('button', { textContent: 'Restore All' });
    const deleteForeverBtn = new MockElement('button', { textContent: 'Delete Forever' });
    libraryPanel.appendChild(playlistsBtn);
    libraryPanel.appendChild(likedBtn);
    libraryPanel.appendChild(sortByLabel);
    libraryPanel.appendChild(restoreAllBtn);
    libraryPanel.appendChild(deleteForeverBtn);

    // 8. Settings 账户中心面板
    const settingsPanel = new MockElement('div', { className: 'settings-panel' });
    const subTitle = new MockElement('h3', { textContent: 'Subscription' });
    const audioQualityLabel = new MockElement('label', { textContent: 'Audio Quality:' });
    const darkThemeBtn = new MockElement('button', { textContent: 'Dark' });
    settingsPanel.appendChild(subTitle);
    settingsPanel.appendChild(audioQualityLabel);
    settingsPanel.appendChild(darkThemeBtn);

    multiPanelsRoot.appendChild(soundsModal);
    multiPanelsRoot.appendChild(uploadModal);
    multiPanelsRoot.appendChild(extendModal);
    multiPanelsRoot.appendChild(editorModal);
    multiPanelsRoot.appendChild(stemsModal);
    multiPanelsRoot.appendChild(contextMenu);
    multiPanelsRoot.appendChild(libraryPanel);
    multiPanelsRoot.appendChild(settingsPanel);
    mockDoc.body.appendChild(multiPanelsRoot);

    engine.scanAndInject();

    // 验证各面板汉化挂载
    assert.strictEqual(loopBtn.querySelector('.suno-copilot-nav-zh').textContent, '循环播放', 'FAIL: Sounds Loop 汉化不匹配');
    assert.strictEqual(oneShotBtn.querySelector('.suno-copilot-nav-zh').textContent, '单次触发', 'FAIL: Sounds One-shot 汉化不匹配');
    assert.strictEqual(pitchLabel.querySelector('.suno-copilot-nav-zh').textContent, '音调高低', 'FAIL: Sounds Pitch: 汉化不匹配');
    assert.strictEqual(keyLabel.querySelector('.suno-copilot-nav-zh').textContent, '调性', 'FAIL: Sounds Key: 汉化不匹配');
    assert.strictEqual(bpmLabel.querySelector('.suno-copilot-nav-zh').textContent, '节拍速度 (BPM)', 'FAIL: Sounds BPM: 汉化不匹配');

    assert.strictEqual(uploadHeader.querySelector('.suno-copilot-nav-zh').textContent, '上传参考音频', 'FAIL: Upload Audio 标题汉化不匹配');
    assert.strictEqual(recordBtn.querySelector('.suno-copilot-nav-zh').textContent, '录制音频', 'FAIL: Record Audio 汉化不匹配');
    assert.strictEqual(chooseFileBtn.querySelector('.suno-copilot-nav-zh').textContent, '选择文件', 'FAIL: Choose File 汉化不匹配');
    assert.strictEqual(startTimeLabel.querySelector('.suno-copilot-nav-zh').textContent, '起始时间', 'FAIL: Start Time: 汉化不匹配');

    assert.strictEqual(extendAtLabel.querySelector('.suno-copilot-nav-zh').textContent, '续写起始点', 'FAIL: Extend At: 汉化不匹配');
    assert.strictEqual(fullSongBtn.querySelector('.suno-copilot-nav-zh').textContent, '合并整首歌', 'FAIL: Get Whole Song 汉化不匹配');
    assert.strictEqual(part1Btn.querySelector('.suno-copilot-nav-zh').textContent, '第 1 段', 'FAIL: Part 1 汉化不匹配');

    assert.strictEqual(cropBtn.querySelector('.suno-copilot-nav-zh').textContent, '裁剪音频时长', 'FAIL: Crop Audio 汉化不匹配');
    assert.strictEqual(replaceBtn.querySelector('.suno-copilot-nav-zh').textContent, '局部重做指定片段', 'FAIL: Replace Section 汉化不匹配');
    assert.strictEqual(speedLabel.querySelector('.suno-copilot-nav-zh').textContent, '播放速度', 'FAIL: Playback Speed: 汉化不匹配');
    assert.strictEqual(fadeInBtn.querySelector('.suno-copilot-nav-zh').textContent, '开头声音渐渐变大', 'FAIL: Fade In 汉化不匹配');

    assert.strictEqual(getStemsBtn.querySelector('.suno-copilot-nav-zh').textContent, '分轨提取', 'FAIL: Get Stems 汉化不匹配');
    assert.strictEqual(vocalStemLabel.querySelector('.suno-copilot-nav-zh').textContent, '人声分轨', 'FAIL: Vocal Stem 汉化不匹配');
    assert.strictEqual(instStemLabel.querySelector('.suno-copilot-nav-zh').textContent, '伴奏分轨', 'FAIL: Instrumental Stem 汉化不匹配');

    assert.strictEqual(reusePromptBtn.querySelector('.suno-copilot-nav-zh').textContent, '一键复用提示词与参数', 'FAIL: Reuse Prompt 汉化不匹配');
    assert.strictEqual(coverSongBtn.querySelector('.suno-copilot-nav-zh').textContent, '根据旋律翻唱新风格', 'FAIL: Cover Song 汉化不匹配');
    assert.strictEqual(downloadAudioBtn.querySelector('.suno-copilot-nav-zh').textContent, '下载音频 (MP3)', 'FAIL: Download Audio 汉化不匹配');
    assert.strictEqual(shareBtn.querySelector('.suno-copilot-nav-zh').textContent, '分享歌曲', 'FAIL: Share 汉化不匹配');
    assert.strictEqual(moveToTrashBtn.querySelector('.suno-copilot-nav-zh').textContent, '移入回收站', 'FAIL: Move to Trash 汉化不匹配');

    assert.strictEqual(playlistsBtn.querySelector('.suno-copilot-nav-zh').textContent, '歌单列表', 'FAIL: Playlists 汉化不匹配');
    assert.strictEqual(likedBtn.querySelector('.suno-copilot-nav-zh').textContent, '我喜欢的歌', 'FAIL: Liked 汉化不匹配');
    assert.strictEqual(sortByLabel.querySelector('.suno-copilot-nav-zh').textContent, '排序方式', 'FAIL: Sort by: 汉化不匹配');
    assert.strictEqual(restoreAllBtn.querySelector('.suno-copilot-nav-zh').textContent, '全部恢复', 'FAIL: Restore All 汉化不匹配');
    assert.strictEqual(deleteForeverBtn.querySelector('.suno-copilot-nav-zh').textContent, '彻底粉碎删除', 'FAIL: Delete Forever 汉化不匹配');

    assert.strictEqual(subTitle.querySelector('.suno-copilot-nav-zh').textContent, '会员订阅', 'FAIL: Subscription 标题汉化不匹配');
    assert.strictEqual(audioQualityLabel.querySelector('.suno-copilot-nav-zh').textContent, '导出音质', 'FAIL: Audio Quality: 汉化不匹配');
    assert.strictEqual(darkThemeBtn.querySelector('.suno-copilot-nav-zh').textContent, '深色模式', 'FAIL: Dark 汉化不匹配');

    console.log('✅ 断言 16 通过: 全界面所有面板（Sounds、Upload、Extend、Editor、Stems、Menu、Library、Settings）高级参数与折叠控制汉化 100% 全覆盖！');
    testsPassed++;
  }

  // ==============================================================
  // 断言 17：【Labs 实验工坊、Listen and Rank (听歌评分赚积分) 与个人主页设置汉化 100% 覆盖】
  // 验证评测题干、超长说明段落 (<p> 标签)、Clip A/B、动态试听倒计时、动态积分、快捷键、
  // 占位符、以及 Profile / Account 设置面板的所有中英文双语呈现
  // ==============================================================
  {
    const labsRoot = new MockElement('div', { className: 'suno-labs-listen-and-rank' });

    // 1. 顶层标题与动态积分徽标
    const headerTitle = new MockElement('h2', { textContent: 'Which clip better matches the prompt?' });
    const earnCreditsBadge = new MockElement('span', { className: 'badge', textContent: 'Earn 5 credits' });

    // 2. 评测长说明段落 (<p>)
    const instructionP = new MockElement('p', {
      textContent: "Two clips, A and B. Listen to both, then pick the one that better answers the prompt. If you're not sure, pick either one. Every response helps Suno improve!"
    });

    // 3. 标签与片段对比区
    const tagsLabel = new MockElement('span', { textContent: 'Tags:' });
    const clipABtn = new MockElement('button', { textContent: 'Clip A' });
    const clipBBtn = new MockElement('button', { textContent: 'Clip B' });
    const countdownSpan = new MockElement('span', { className: 'countdown-timer', textContent: 'Listen for 2 more seconds' });

    // 4. 评价选择区与选项
    const criteriaLabel = new MockElement('label', { textContent: 'prompt_following' });
    const neitherBtn = new MockElement('button', { textContent: 'Neither' });
    const explainInput = new MockElement('textarea', { placeholder: 'Optional: explain your selection(s)' });

    // 5. 底部操作按钮与快捷键栏
    const submitBtn = new MockElement('button', { textContent: 'Submit' });
    const skipBtn = new MockElement('button', { textContent: 'Skip' });
    const reportBtn = new MockElement('button', { textContent: 'Report a problem' });
    const skipProjectBtn = new MockElement('button', { textContent: 'Skip project' });
    const shortcutsTitle = new MockElement('span', { textContent: 'Shortcuts' });
    const shortcut1 = new MockElement('span', { textContent: '1: Clip A' });
    const shortcutEnter = new MockElement('span', { textContent: 'Enter: Submit' });

    labsRoot.appendChild(headerTitle);
    labsRoot.appendChild(earnCreditsBadge);
    labsRoot.appendChild(instructionP);
    labsRoot.appendChild(tagsLabel);
    labsRoot.appendChild(clipABtn);
    labsRoot.appendChild(clipBBtn);
    labsRoot.appendChild(countdownSpan);
    labsRoot.appendChild(criteriaLabel);
    labsRoot.appendChild(neitherBtn);
    labsRoot.appendChild(explainInput);
    labsRoot.appendChild(submitBtn);
    labsRoot.appendChild(skipBtn);
    labsRoot.appendChild(reportBtn);
    labsRoot.appendChild(skipProjectBtn);
    labsRoot.appendChild(shortcutsTitle);
    labsRoot.appendChild(shortcut1);
    labsRoot.appendChild(shortcutEnter);

    // 6. Profile 个人资料设置面板
    const profilePanel = new MockElement('div', { className: 'profile-settings-page' });
    const editProfileTitle = new MockElement('h2', { textContent: 'Edit Profile' });
    const publicProfileBtn = new MockElement('button', { textContent: 'Public Profile' });
    const handleLabel = new MockElement('label', { textContent: 'Handle' });
    const bioInput = new MockElement('textarea', { placeholder: 'Tell the world about yourself' });
    const handleInput = new MockElement('input', { placeholder: 'Your handle and profile URL' });
    const saveProfileBtn = new MockElement('button', { textContent: 'Save Profile' });
    const connectedAccountsTitle = new MockElement('h3', { textContent: 'Connected Accounts' });
    const invoicesBtn = new MockElement('button', { textContent: 'Invoices' });

    profilePanel.appendChild(editProfileTitle);
    profilePanel.appendChild(publicProfileBtn);
    profilePanel.appendChild(handleLabel);
    profilePanel.appendChild(bioInput);
    profilePanel.appendChild(handleInput);
    profilePanel.appendChild(saveProfileBtn);
    profilePanel.appendChild(connectedAccountsTitle);
    profilePanel.appendChild(invoicesBtn);

    mockDoc.body.appendChild(labsRoot);
    mockDoc.body.appendChild(profilePanel);

    engine.scanAndInject();

    // 验证 Labs 静态与动态内容汉化
    assert(headerTitle.querySelector('.suno-copilot-nav-zh'), 'FAIL: Which clip 题干未汉化');
    assert.strictEqual(headerTitle.querySelector('.suno-copilot-nav-zh').textContent, '哪一个片段更符合提示词要求？', 'FAIL: Which clip 汉化文本不匹配');

    assert(earnCreditsBadge.querySelector('.suno-copilot-nav-zh'), 'FAIL: Earn 5 credits 未动态汉化');
    assert.strictEqual(earnCreditsBadge.querySelector('.suno-copilot-nav-zh').textContent, '赚取 5 积分', 'FAIL: Earn 5 credits 汉化文本不匹配');

    assert(instructionP.querySelector('.suno-copilot-nav-zh'), 'FAIL: instruction <p> 说明段落未汉化');
    assert(instructionP.querySelector('.suno-copilot-nav-zh').classList.contains('block'), 'FAIL: 长段落汉化未附加 block 类样式');
    assert.strictEqual(instructionP.querySelector('.suno-copilot-nav-zh').textContent, '片段 A 与 B。请试听两者，并选出更能体现提示词的片段。如果不确定，任选其一即可。每一次回答都能帮助 Suno 进化！', 'FAIL: instruction 汉化文本不匹配');

    assert.strictEqual(tagsLabel.querySelector('.suno-copilot-nav-zh').textContent, '风格标签', 'FAIL: Tags: 汉化不匹配');
    assert.strictEqual(clipABtn.querySelector('.suno-copilot-nav-zh').textContent, '片段 A', 'FAIL: Clip A 汉化不匹配');
    assert.strictEqual(clipBBtn.querySelector('.suno-copilot-nav-zh').textContent, '片段 B', 'FAIL: Clip B 汉化不匹配');

    assert(countdownSpan.querySelector('.suno-copilot-nav-zh'), 'FAIL: 倒计时未动态汉化');
    assert.strictEqual(countdownSpan.querySelector('.suno-copilot-nav-zh').textContent, '还需试听 2 秒', 'FAIL: 倒计时汉化文本不匹配');

    // 模拟倒计时动态变化：Listen for 2 more seconds -> Listen for 1 more second
    // 验证动态更新而无需刷新页面
    countdownSpan.children[0].textContent_ = 'Listen for 1 more second';
    engine.scanNavigationAndMenus();
    assert.strictEqual(countdownSpan.querySelector('.suno-copilot-nav-zh').textContent, '还需试听 1 秒', 'FAIL: 动态倒计时更新后未同步刷新中文');

    assert.strictEqual(criteriaLabel.querySelector('.suno-copilot-nav-zh').textContent, '提示词贴合度评分', 'FAIL: prompt_following 汉化不匹配');
    assert.strictEqual(neitherBtn.querySelector('.suno-copilot-nav-zh').textContent, '都不符合', 'FAIL: Neither 汉化不匹配');
    assert(explainInput.placeholder.includes('选填：写下你选择该片段的理由'), 'FAIL: explain selection 占位符未汉化');

    assert.strictEqual(submitBtn.querySelector('.suno-copilot-nav-zh').textContent, '提交', 'FAIL: Submit 汉化不匹配');
    assert.strictEqual(skipBtn.querySelector('.suno-copilot-nav-zh').textContent, '跳过', 'FAIL: Skip 汉化不匹配');
    assert.strictEqual(reportBtn.querySelector('.suno-copilot-nav-zh').textContent, '反馈问题', 'FAIL: Report a problem 汉化不匹配');
    assert.strictEqual(skipProjectBtn.querySelector('.suno-copilot-nav-zh').textContent, '跳过此任务', 'FAIL: Skip project 汉化不匹配');
    assert.strictEqual(shortcutsTitle.querySelector('.suno-copilot-nav-zh').textContent, '快捷键指南', 'FAIL: Shortcuts 汉化不匹配');
    assert.strictEqual(shortcut1.querySelector('.suno-copilot-nav-zh').textContent, '1: 片段 A', 'FAIL: 1: Clip A 汉化不匹配');
    assert.strictEqual(shortcutEnter.querySelector('.suno-copilot-nav-zh').textContent, 'Enter: 提交', 'FAIL: Enter: Submit 汉化不匹配');

    // 验证 Profile 设置面板汉化
    assert.strictEqual(editProfileTitle.querySelector('.suno-copilot-nav-zh').textContent, '编辑个人资料', 'FAIL: Edit Profile 汉化不匹配');
    assert.strictEqual(publicProfileBtn.querySelector('.suno-copilot-nav-zh').textContent, '公开个人主页', 'FAIL: Public Profile 汉化不匹配');
    assert.strictEqual(handleLabel.querySelector('.suno-copilot-nav-zh').textContent, '个性标识 (@)', 'FAIL: Handle 汉化不匹配');
    assert(bioInput.placeholder.includes('用一句话介绍你自己'), 'FAIL: Tell the world about yourself 占位符未汉化');
    assert(handleInput.placeholder.includes('你的个性标识与主页地址'), 'FAIL: Your handle and profile URL 占位符未汉化');
    assert.strictEqual(saveProfileBtn.querySelector('.suno-copilot-nav-zh').textContent, '保存个人资料', 'FAIL: Save Profile 汉化不匹配');
    assert.strictEqual(connectedAccountsTitle.querySelector('.suno-copilot-nav-zh').textContent, '关联第三方账号', 'FAIL: Connected Accounts 汉化不匹配');
    assert.strictEqual(invoicesBtn.querySelector('.suno-copilot-nav-zh').textContent, '发票与收据', 'FAIL: Invoices 汉化不匹配');

    console.log('✅ 断言 17 通过: Labs (Listen and Rank 试听打分评测) 动态倒计时/积分/长说明与个人主页/账户设置 100% 全覆盖！');
    testsPassed++;
  }
