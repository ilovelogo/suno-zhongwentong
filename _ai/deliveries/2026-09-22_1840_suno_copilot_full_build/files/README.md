...
5. **`src/data/glossary.json`**  
   *独立核心知识库资产*：与业务代码完全解耦的 50 个全维结构化词目，可随时扩展。
...
8. **`scripts/` 工具链**  
   ...
   - `export_review.js`：一键将词库同步导出为 Markdown 与 Excel CSV 评审表。

---

## 🛠️ 一键自动化构建与更新指令

本项目纯原生零外部依赖，使用 Node.js 即可完成构建与导出：

```bash
# 1. 一键编译并同步 dist 产物
node scripts/build.js

# 2. 一键导出最新的 Markdown 与 Excel CSV 专家校对表
...
4. 在 Kiwi 浏览器中访问 `https://suno.com/`：
   - 按钮旁会自动出现中文小标题与高亮 `[?]` 徽标；
   - 使用 **S-Pen 笔尖悬停**在徽标上方 0.15 秒，即可预览三层白话卡片；
...
- **三、演唱风格与人声情绪 (Vocals & Performance)**：`[Whisper]`（耳语气声）、`[Belting]`（高亢怒音）、`[Falsetto]`（空灵假音）、`[Spoken Word]`（念白独白）、`[Harmonies]`（多声部和声）、`[Vibrato]`（情感颤音）、`[Ad-lib]`（即兴垫音）、`[Call and Response]`（呼应对唱）、`[Chant]`（万人呐喊）等。
...
本项目所有核心知识均保存在 `Music_Expert_Review_Sheet.md` 和 `Music_Expert_Review_Sheet.csv` 中。  
您可以直接将该文件转发给音乐制作人或音乐教师：
1. **Markdown 格式**：适合习惯使用 Notion、Typora 或 GitHub 的专业人士；
2. **CSV 表格格式**：适合习惯使用 Excel / WPS 的老师，在每条词目右侧的“专家校对与修改意见”一栏直接输入；
3. 校对意见收齐后，通过脚本即可一键反向合并至 `glossary.json`，完成词库的持续进阶升级！
