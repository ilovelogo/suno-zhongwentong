...
class SunoCopilotEngine {
  ...
  translateMusicDescription(text, onDynamicComplete = null) {
    ...
    // 2. 复合短语拆分：支持逗号、分号以及常见连词 (and, with, feat)
    // 例如：tight four-on-the-floor groove and surprise kazoo
    // 自然解构为 [tight four-on-the-floor groove] 和 [surprise kazoo]
    const rawChunks = trimmed.split(/[,;]+|\s+(?:and|with|feat\.?|featuring)\s+/i).map(s => s.trim()).filter(Boolean);
    ...
    for (let i = 0; i < rawChunks.length; i++) {
      ...
      // 2.2 段内混合短语：采用 Token Masking（占位符保护）
      // 防止一段内已知词后连着未知生词时，生词带跑整段被机翻成“地板上的四个”
      ...
      // 如果全部被词典覆盖
      if (tokenMap.size > 0 && !/[a-zA-Z]/.test(masked.replace(/SCTERM\d+X/gi, ''))) {
        let fullZh = masked;
        ...
        // 关键防护：段落内既有已知专业词（如 tight four-on-the-floor groove）又有未知生词（如 surprise kazoo）
        ...
        // 完全未知生词段
        resolvedChunks.push({ index: i, text: chunk, isLocked: false, maskedText: chunk, tokenMap: null });
        untranslatedTasks.push({ index: i, text: chunk, tokenMap: null });
      }
    }
    ...
    if (untranslatedTasks.length > 0 && typeof this.fetchDynamicTranslation === 'function') {
      ...
      Promise.all(tasks).then((results) => {
        ...
        if (hasUpdate) {
          const finalValid = resolvedChunks
