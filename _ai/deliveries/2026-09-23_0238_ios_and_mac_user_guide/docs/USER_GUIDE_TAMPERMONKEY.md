...
## 第二部分：安卓平板端（三星 Galaxy Tab / S-Pen 触控优化）

### 步骤 1：在平板上安装推荐浏览器
原生安卓 Chrome 默认不支持安装扩展，推荐使用：
1. **首选推荐：Kiwi Browser（猕猴桃浏览器）**
   - 纯正 Chromium 内核，**完美支持 Chrome Web Store 扩展，对三星 S-Pen 笔尖悬停（Hover / Pointerenter）支持极佳**。
2. **备选推荐：Firefox for Android（火狐安卓版）**

### 步骤 2：安装 Tampermonkey
在 Kiwi Browser 中点击右上角菜单（`⋮`）->「Extensions（扩展程序）」-> 打开「Chrome 网上应用店」搜索并安装 `Tampermonkey`。

### 步骤 3：导入脚本
将 `suno-copilot.user.js` 传到平板，在 Kiwi 浏览器中直接打开该文件或在 Tampermonkey 管理面板选择「从文件导入」即可。
